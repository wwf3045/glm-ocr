...
| 方案 | 最适合 | 劣势 | 部署方式 |
|------|--------|------|----------|
| **GLM-OCR**（智谱 ZhipuAI） | 结构化文档、公式、专业术语。0.9B 参数，~1.86页/秒，API 仅 0.2元/百万 token（传统 OCR 的 1/10） | 不能提取图片，无 bbox 输出，模糊文字有幻觉 | 云端 API / VLLM 本地 |
| **PaddleOCR VL 1.5**（百度 Baidu） | 手写体、表格、物理畸变图片 | CUDA 依赖地狱，逻辑重组能力弱 | 仅本地 GPU |
| **MinerU**（OpenDataLab） | 排版简单的干净 PDF | 复杂版面错字多 | 仅本地 GPU |
| **DeepSeek OCR2**（深度求索 DeepSeek） | 表格（信息零丢失） | 公式错误，图片丢弃 | 云端 API |
| **云知声 U1**（Unisound） | 医疗/病历文档，字段级定位溯源 | 较新，社区实测较少 | 云端 API |
